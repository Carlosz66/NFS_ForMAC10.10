...
- (void) createSampleData
{
    ...
    sampleData2 = [[NSMutableArray array]retain];
...
- (void) normalizeSample
{
    ...
    normalizedData2 = [[NSMutableArray array]retain];
	float min = MAXFLOAT;
	float max = -MAXFLOAT;
...
//	NSLog(@"Min: %f, Max: %f, Delta: %Lf", min,  max, delta);
    ...
    for(NSNumber *num in sampleData2) {
        float val = num.floatValue;
        if(val > 1.0) val = 1.0;
        if(val < 0.0) val = 0.0;
        NSNumber *nval = [NSNumber numberWithFloat:val];
        [normalizedData2 addObject:nval];
    }
	[sampleData release];
    [sampleData2 release];
	sampleData = nil;
...
//		int c = 0;
		...
		//}
        for(int i=0;i<extAFNumChannels;i++){
            NSNumber *nMaxValue = [NSNumber numberWithFloat:maxValues[i]];
            if(i==0)
                [sampleData addObject:nMaxValue];
            else if(i==1)
                [sampleData2 addObject:nMaxValue];
        }
	}
}
...
- (float *)dataForResolution2:(int)pixelWide lenght:(int *)length
{
    //	int samplePerSec = 44100.0 / binSize;
    //	int secPerPixel = (int)lengthInSec / (int)pixelWide;
    if(extAFNumChannels==1){
        *length=0;
        return NULL;
    }
    int rangeLength = normalizedData2.count / pixelWide + 1;
    int retLength = pixelWide;
    float *ret = (float *)calloc(sizeof(float),retLength);
    int k = 0;
    for(int r = 0; r < normalizedData2.count; r += rangeLength) {
        float valMax = 0;
        for(int j = 0; j < rangeLength; j++) {
            int idx = r + j;
            if(idx < normalizedData2.count) {
                NSNumber *nVal = [normalizedData2 objectAtIndex:idx];
                float val = nVal.floatValue;
                if(valMax < val) valMax = val;
            }
        }
